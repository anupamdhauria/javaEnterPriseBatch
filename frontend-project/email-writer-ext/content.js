...
function createAIButton() {
    ...
    button.setAttribute('data-tooltrip', 'Generate AI Reply');
    return button;
}
...
function injectButton() {
    ...
    button.addEventListener('click', async () => {
        try {
            ...
            const response = await fetch('http://localhost:9999/api/email/generate', {
                method: 'POST',
                headers: {
            ...
            if (composeBox) {
                composeBox.focus();
                const formattedReply = generateReply
                    .split(/\n\s*\n/)                 // split paragraphs
                    .map(paragraph => {
